...
function d = build_parameters(d)
    ...
    d.p.u1_min = -(50/180)*pi;
    d.p.u1_max = (50/180)*pi;
    d.p.u2_min = -500;
    d.p.u2_max = 2000;
...
function d = check_car_condition(d,k)
    ...
    % enforce control input constraints
    d.s.u(1,k) = min(max(d.s.u(1,k),d.p.u1_min),d.p.u1_max);
    d.s.u(2,k) = min(max(d.s.u(2,k),d.p.u2_min),d.p.u2_max);
    
    % check for crashing into wall
...
function dxdt = car_equations_of_motion(t,x,u,d)
    ...
    dxdt(4,1) = (FxF * cos(ALPHAT - DELTA) + FxR * cos(ALPHAT) + ...
        FyF * sin(ALPHAT - DELTA) + FyR * sin(ALPHAT) - ...
        d.p.drag_coef*v*abs(v))/d.c.m;
