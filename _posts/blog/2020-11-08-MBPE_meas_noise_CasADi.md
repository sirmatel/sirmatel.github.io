...
To see why solving the convex approximation can potentially lead to a good initial guess for the original problem, we can plot the nonconvex objective function values from before together with those of the convex approximation (which can be obtained by solving the convex approaximation with the parameter fixed to a value, for a set of values):
